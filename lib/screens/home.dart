...
import 'package:fluttercommerce/logic/home/bloc/home_bloc.dart';
...
import 'package:fluttercommerce/screens/add_product.dart';
...
import 'package:fluttercommerce/screens/profile_screen.dart';
...
class _HomeState extends State<Home> {
  ...
  _setBlocEvent() async {
    ...
    BlocProvider.of<HomeBloc>(context).add(UpdateHome(token: 'token'));
  ...
  _setCategoryImages(List<Category> categories) {
    categoryImgs = [];
    categoryNames = [];
    for (var category in categories) {
      categoryImgs.add(category.image);
      ...
      child: Scaffold(
        ...
        body: BlocBuilder<HomeBloc, HomeState>(
          builder: (context, state) {
            ...
            } else if (state is HomeLodaded) {
              products = state.products;
              _setCategoryImages(state.categories);
              return TabBarView(
                children: [
                  Container(
                      ...
                      child: Column(
                        children: <Widget>[
                          CategoriesListView(
                            userId: _userId,
                            title: "YOUR TITLES",
                            categories: categoryImgs,
                            categoryTitle: categoryNames,
                          ...
                          buildTrending(_userId == null
                              ? state.products
                              : state.products
                                  .where((product) =>
                                      product.owner != int.parse(_userId))
                                  .toList()),
                          Padding(
                            padding: const EdgeInsets.all(6.0),
                            child: Row(
  ...
  leftDrawerMenu() {
    ...
    return InkWell(
      onTap: () {
        Nav.route(context, ProfileScreen());
      },
      child: Container(
        child: ListView(
          ...
          children: <Widget>[
            ...
            ListTile(
              ...
              title: Text('My Orders',
                  style: TextStyle(
                      ...
                      color: blackColor)),
              onTap: () {
                Nav.route(context, MyOrderScreen(products: products));
              },
            ),
            ListTile(
              leading: Icon(Icons.shopping_cart, color: blackColor),
            ...
            ListTile(
              leading: Icon(Feather.getIconData('message-circle'),
                  color: blackColor),
              title: Text('Support',
                  style: TextStyle(
                      fontSize: 16,
              ...
              onTap: () {
                Nav.route(context,
                    ProductListScreen(userId: _userId, title: 'My Product'));
              },
            ),
            ListTile(
              leading:
