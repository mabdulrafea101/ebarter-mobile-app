...
class MyProductProvider {
  ...
  Future<List<Product>> getProductData(String token) async {
    print('$token token');
    final url =
        Uri.parse("${CustomStrings.baseUrl}/api/product/?owner_id=$token");
  ...
  Future<List<Category>> getCategoryData(String token) async {
    print('$token token');
    final url = Uri.parse("${CustomStrings.baseUrl}/api/product/category/");
    final response = await client.get(url
        // headers: {
        //   'Content-Type': 'application/json',
        //   'Accept': 'application/json',
