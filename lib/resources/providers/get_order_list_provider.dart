...
class GetOrderListProvider {
  http.Client client = http.Client();

  Future<List<Order>> getProductData(String token) async {
    print('$token token');
    final url = Uri.parse("${CustomStrings.baseUrl}/api/order");
    ...
    if (response.statusCode == 200) {
      OrderList orders = OrderList.fromJson(json.decode(response.body));
