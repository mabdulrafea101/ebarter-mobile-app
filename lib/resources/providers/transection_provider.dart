...
class TransectionProvider {
  http.Client client = http.Client();

  Future<TransectionList> getData(String token) async {
    print('$token token');
    final url = Uri.parse("${CustomStrings.baseUrl}/api/payments/transection/");
    ...
    if (response.statusCode == 200) {
      TransectionList transectionList =
          TransectionList.fromJson(json.decode(response.body));
